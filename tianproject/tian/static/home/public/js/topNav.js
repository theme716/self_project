...
function cartAddMin(){
      ...
      //页面底部显示初始值
      //初始商品总数量
      var a = $('.cart-product').children().find('input')
      all = 0
      for(var i=0;i<=a.length-1;i++){
        aa = parseInt(a.eq(i).val())
        if(aa==1){
            $('.cart-product').children().find('.mz-adder-subtract').eq(i).attr('disabled',"true")

        }

        all += parseInt(a.eq(i).val())
...
//              $nSub.addClass('disabled');
              $nSub.attr('disabled',true)
              $nInput.val(1);
           }else if(num>1){
              $nInput.val(num);
              $nSub.attr('disabled',false)
           }
           if(num<5){
